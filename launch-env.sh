...
K8S_DOCKER_IMAGE=${K8S_DOCKER_IMAGE:-'ucsdets/scipy-ml-notebook:2020.2.9'}
K8S_ENTRYPOINT=${K8S_ENTRYPOINT:-"/run_jupyter.sh"}
...
PROTO_K8S_EXTRA_ENV=$(cat <<'EOM'
    # no extra environment vars
EOM
)
K8S_EXTRA_ENV=${K8S_EXTRA_ENV:-${PROTO_K8S_EXTRA_ENV}}
...
while getopts ":n:hi:c:g:m:dSsBbBv:e:p:P:xqrft" opt; do
  case $opt in
    d) 
    # --dry-run
    ...
    f)
      ...
      ;;
    h)
      echo "Usage: ${K8S_CONFIG_SOURCE} [-n nodenum] [-i dockerepo/image] [-c #core] [-g #gpu] [-m #gb-ram] [-v (1080ti|1070ti|2080ti)] [-b]" >&2
      exit 0
      ;;
    c)
      if ! [[ $OPTARG =~ ^[0-9]+$ ]]; then
    ...
    p)
      ...
      ;;
    P)
      if ! [[ $OPTARG =~ ^(IfNotPresent|Always)$ ]]; then
        echo "Error: unknown imagePullPolicy $OPTARG; valid choices: IfNotPresent,Always" >&2; exit 1
      fi
      K8S_IMAGE_PULL_POLICY=$OPTARG
      ;;
    e)
    ...
    i)
      ...
      ;;
    v)
      if ! [[ $OPTARG =~ ^(1080ti|1070ti|2080ti)$ ]]; then
         echo "Error: unknown gpu-model $OPTARG; valid choices: 1080ti,1070ti,2080ti" >&2; exit 1
      fi
      AFFINITY_REQUIRED_OP='In'
      AFFINITY_REQUIRED_VALUE=$OPTARG
      ;;
    n)
...
#######################################################
# Steer CPU-only jobs towards our CPU-only nodes
if (( $K8S_NUM_GPU == 0 )); then
  AFFINITY_REQUIRED_KEY='admitcpupods'
  AFFINITY_REQUIRED_OP='In'
  AFFINITY_REQUIRED_VALUE='"true"'
  AFFINITY_PREFERRED_KEY='gputype'
  AFFINITY_PREFERRED_OP='In'
  AFFINITY_PREFERRED_VALUE='none'
else
  AFFINITY_REQUIRED_KEY='gputype'
  AFFINITY_REQUIRED_OP=${AFFINITY_REQUIRED_OP:-'NotIn'}
  AFFINITY_REQUIRED_VALUE=${AFFINITY_REQUIRED_VALUE:-'none'}
  AFFINITY_PREFERRED_KEY='admitcpupods'
  AFFINITY_PREFERRED_OP='NotIn'
  AFFINITY_PREFERRED_VALUE='"true"'
fi

main () {
	# NOTE: This pod name could be more descriptive (e.g. based off container being run)
	POD_NAME=${POD_NAME:-"${K8S_USERNAME}-$$"}
	...
	if [ "$PROXY_ENABLED" = "YES" ]; then 
        ...
        SOCAT_PID=""
        OFFSET=0
        for dest_port in ${PROXY_PORT//:/ }; do
		    SOCAT_PORT=$(findport $OFFSET)
		    if [[ ! $SOCAT_PORT ]]; then
		    ...
		    kubectl port-forward ${POD_NAME} ${SOCAT_PORT}:${dest_port} < /dev/null > /dev/null 2>&1 &
            ...
            echo "Forwarding TCP port http://${PROXY_IP}:${SOCAT_PORT} to container port ${dest_port}"
...
function kill_kube_fwd() {
    for test_pid in ${KUBE_FWD_PID}; do
	    kill -QUIT ${test_pid} 2>/dev/null
    done
}

function check_socat() {
    for test_pid in ${SOCAT_PID}; do
        kill -0 ${test_pid} 2>/dev/null || return 1
    done
    return 0
}
...
function pod_conditions() {
  kubectl get events --field-selector involvedObject.name=${POD_NAME} -o jsonpath='{.items[-1].message}'
}
...
spec:
  ...
  affinity:
    nodeAffinity:
      requiredDuringSchedulingIgnoredDuringExecution:
        nodeSelectorTerms:
        - matchExpressions:
          - key: ${AFFINITY_REQUIRED_KEY}
            operator: ${AFFINITY_REQUIRED_OP}
            values:
            - ${AFFINITY_REQUIRED_VALUE}
      preferredDuringSchedulingIgnoredDuringExecution:
      - weight: 10
        preference:
          matchExpressions:
          - key: ${AFFINITY_PREFERRED_KEY}
            operator: ${AFFINITY_PREFERRED_OP}
            values:
            - ${AFFINITY_PREFERRED_VALUE}
  nodeSelector:
  ...
  - args:
    ...
    - /bin/sh
    - -c
    image: ucsdets/k8s-support:2020.3.1
...
findport() {
	...
	TGT=""
	for ((i= $(( 8000 + ($K8S_UID % 12500) + $1 )) ;i<=25000;i++)); do 
